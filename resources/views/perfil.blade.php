@extends('layouts.app')
@section('content')
    <div id="cards" class="flex flex-col gap-5 px-0 md:px-16 opacity-0 translate-y-4 transition-all duration-700">
        ...
        <a href="{{ route('equipe.edit', $user->id) }}"
            ...
            <img src="{{ asset('frontend/img/edit.png') }}" class="w-8 h-8 hidden md:block">
        </a>
        <div>
            @if ($user->hasRole(['Desenvolvedor', 'Franqueadora']))
                <!-- Bloco para Desenvolvedor -->
                <div
                    class="flex flex-row items-center justify-between bg-white rounded-xl py-6 px-8 border border-gray-200">
                    ...
                    <select id="unidadeSelect"
                        ...
                        @foreach ($unidades as $unidade)
                            <option value="{{ $unidade->id }}" {{ $user->unidade_id == $unidade->id ? 'selected' : '' }}>
                                {{ $unidade->cidade }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div>
                    ...
                    <select id="roleSelect"
                        ...
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}"
                                {{ $user->hasRole($role->name, 'api') ? 'selected' : '' }}>
                                {{ ucfirst($role->name) }}
                            </option>
                        @endforeach
                    </select>
            ...
            @else
                <!-- Bloco para Colaborador ou outros cargos -->
                <div
                    class="flex flex-row items-center justify-between bg-white rounded-xl py-6 px-8 border border-gray-200">
                    <div class="flex flex-row items-center gap-2">
                        <img src="{{ asset('frontend/img/cargo.png') }}" class="w-6 h-6">
                    ...
                    <span class="text-gray-600 text-sm">{{ ucfirst($grupo) }}</span>
                </div>
            @endif
        </div>
...
@endsection
{{-- Atualizador de salario --}}
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const userToken = @json($user_token);

        // Atualiza salário
        async function atualizarSalario() {
            try {
                const response = await fetch('https://rh.taiksu.com.br/salario', {
                    headers: {
                ...
                // Converte para número (já que a API retorna string)
                const salarioBruto = parseFloat(data.salario_bruto) || 0;

                // Formatar para padrão BR
                document.querySelector('#salario').textContent =
                ...
                console.error("Token usado:", userToken);
                document.querySelector('#salario').textContent = "Erro ao carregar";
            }
        }
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        select.addEventListener("change", async (e) => {
            ...
            try {
                const res = await fetch(`/user/${userId}/update-role`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
                ...
                if (res.ok) {
                    ...
                    mostrarNotificacao(data.message || "Erro ao atualizar cargo.", "error");
                }
            } catch (error) {
                console.error(error);
        ...
        function mostrarNotificacao(mensagem, tipo = "success") {
            ...
            toast.className =
                `fixed bottom-6 right-6 ${cores[tipo]} text-white px-5 py-3 rounded-xl shadow-lg opacity-0 translate-y-4 transition-all duration-300 z-50`;
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        selectUnidade.addEventListener("change", async (e) => {
            ...
            try {
                const res = await fetch(`/user/${userId}/update-unidade`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
                ...
                if (res.ok) {
                    mostrarNotificacao(`Unidade atualizada: ${data.unidade_nome}`, "success");
                } else {
                    mostrarNotificacao(data.message || "Erro ao atualizar unidade.", "error");
                }
            } catch (error) {
                console.error(error);
